...
const buttonVariants = cva(
  ...
  {
    variants: {
      // Every variant carries an `active:` state. Touch devices have no hover,
      // and globals.css clears the tap highlight, so without this a tap on a
      // phone produces no visible response at all.
      variant: {
        default:
      ...
      // Touch-first: every size clears 44px on a phone and shrinks to the
      // compact desktop scale at lg. `sm` collapsing to the same height as
      // `default` on mobile is intentional — `sm` exists for desktop density,
      // and on touch there is a floor.
      size: {
