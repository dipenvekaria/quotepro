    ...
    <section className="rounded-xl border border-destructive/30 bg-card shadow-sm">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4">
        <p className="max-w-prose text-sm text-muted-foreground">
          {isOwner
            ? 'Deletes this company and everything in it — customers, quotes, jobs, invoices, your price book, and every login on the team. This happens immediately and cannot be undone.'
        ...
        <Button variant="destructive" onClick={openDialog} className="shrink-0">
          {isOwner ? 'Delete account' : 'Delete my login'}
        </Button>
      </div>
      ...
      <Dialog open={open} onOpenChange={(o) => !deleting && setOpen(o)}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {isOwner ? 'Delete this account?' : 'Delete your login?'}
            </DialogTitle>
            <DialogDescription>
              {isOwner
                ? 'There is no undo and no backup. Everything below is destroyed the moment you confirm.'
            ...
            <div className="space-y-4">
              ...
              {impact.unpaid.count > 0 && impact.scope === 'company' && (
                <Warning>
                  ...
                  {impact.unpaid.count === 1 ? 'invoice' : 'invoices'}. Deleting removes any record
                  of who owes it.
                </Warning>
              )}
              ...
              {impact.stripeConnected && impact.scope === 'company' && (
                <Warning>
                  Stripe is still connected. Payouts already in flight will still arrive, but this
                  removes your record of which job they were for — disconnect in Integrations first
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={confirm} disabled={!matches || deleting}>
              {deleting ? 'Deleting…' : isOwner ? 'Delete everything' : 'Delete my login'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
