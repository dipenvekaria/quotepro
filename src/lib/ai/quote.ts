...
export type GeneratedQuote = {
  ...
  unmet: string[]
  /** The job in 2–5 plain words — card titles, not prose. */
...
const QUOTE_SCHEMA: Schema = {
  ...
  properties: {
    ...
    // The job in a few plain words — what the pipeline card gets titled.
...
async function realGenerate(
  ...
  // 2–5 words means 2–5 words: a sentence here becomes a card title that
  // wraps three lines. Over-length answers are cut at the word boundary.
  const job_name =
    typeof data.job_name === 'string' && data.job_name.trim()
      ? data.job_name.trim().split(/\s+/).slice(0, 5).join(' ')
      : null

  return { line_items, reasoning, sources, model: result.model, usage: result.usage, questions, unmet, job_name }
}
