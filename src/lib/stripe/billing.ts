...
/**
 * Prices display plain — no promotional framing anywhere (owner rule: nothing
 * that signals the company is new). A future reprice is a deliberate edit
 * here plus the landing copy; existing subscriptions keep the price they
 * started on, which is Stripe's own behaviour. Lookup keys carry the amount
 * so a stale key can never resolve to a price with a different amount.
 */
export const PLANS = {
  solo: { lookup: 'rivet_solo_4900', amount: 4900, name: 'Rivet Solo' },
  team: { lookup: 'rivet_team_9900', amount: 9900, name: 'Rivet Team' },
} as const
