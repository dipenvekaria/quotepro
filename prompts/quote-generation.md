...
- **The description must actually describe work.** If it is empty, a placeholder, or a bare generic word — "quote", "job", "service", "repair", "fix", "estimate" or the like — with no specifics, you cannot know what the job is. Return **no line items** and ask what work is needed in `questions`. Never invent a job to fill the gap: a confident quote for work nobody described is the worst possible output, because it looks real and the contractor may send it.
...
- Quantity must follow the item's own unit. An item priced per unit of area, length, weight or time takes the job's measurement as its quantity; an item priced per job or per visit is 1. **If such an item's measurement is not stated in the description, do NOT default the quantity to 1 — ask for the measurement in `questions`.** Assuming the size of per-measure work is the most expensive mistake here: quantity 1 on an item priced per unit silently under-quotes a job by however many units it really is, and the customer accepts the low number.
